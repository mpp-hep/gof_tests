  ...
  if doAllQuantiles:
    ...
    for xs in xsecs:
        ...
        for key in sorted(columns.keys()):
            ...
            for i in range(1):
              os.system('combine -M GoodnessOfFit --algo saturated --fixedSignalStrength 0 -d datacard_ws_{PREFIX}_ratio.txt -t 50 --toysFreq -n Ratio_gof_toys_{PREFIX}  --dataset data_obs -s {S} -v 0'.format(PREFIX=prefix,S=40+i))
